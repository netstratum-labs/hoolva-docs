...
<iframe
  ...
  title="Hoolva SDK - Building a simple video conference application from scratch"
  frameborder="0"
...
When videoStream, mixedAudio and screenshareStream are updated, it is passed through streamReceived event .This is only available for react native since binding streams requires streams instead of participant*id [(Stream Bind)](Electron#stream-bind), other updates are updated via \_updateparticipantlist*
.
...
Listeners should be added right after creating MeetingSession. When an event occurs concurrent callback function is called [Meeting events](Electron#meeting-events)
...
The _getStream_ function allows you to access the _MediaStream_ of a particular participant. It returns a _MediaStream_ object for the participant in the meeting.
This function is useful to skip SDK’s binding method and manually manage the stream. Another use case can be when showing a stream in a child window in electron.

```js
meetingsession.getStream(participantsId)
```

**participantId** - array of participant_id - returns videoStream of these participants. If the participantId is empty or contains participants with no video it will return an error.

Returned data type is as follows:

```js
Array<{participant_id:string , stream:Mediastream}>
```

**stream** - stream is a property that holds a MediaStream object, it represents the media stream of the participant.

### Generate Stream

The _generateStream_ function can be used to create a video and/or audio stream. It returns a _MediaStream_ object that can be used to create a stream with a video track, an audio track, or both.

```js
meetingsession.generateStream(participant_id, mute, stream).then((stream) => {
  //received stream with track
})
```
...
```js
generateStream(
    stream: ”audio” | “video” | ”both”
    options: MediaTrackConstraints,
  )
```

**stream** - indicates which stream needs to be created. “audio” creates the audio stream on the track. “video” creates the video stream on the track. “Both” creates both audio and video on single mediaStream.

**Options** - specifies the property of requested streams.
...
**device_Id** - specifying a device ID or an array of device IDs which are acceptable and/or required.

**echoCancellation** - object specifying whether or not echo cancellation is preferred and/or required.

**aspectRatio** - specifying the video aspect ratio or range of aspect ratios which are acceptable and/or required. Eg, for 4:3 give 1.3333 and for 16:9 give 1.7777.

**frameRate** - specifying the frame rate or range of frame rates which are acceptable and/or required.

**height** - specifying the video height or range of heights which are acceptable and/or required.

**width** - specifying the video width or range of widths which are acceptable and/or required.

### Close Stream

The _closeStream_ function is used to destroy a stream that was created using the generateStream function. It is important to use closeStream to remove “camera in use” (the green light next to the camera).

```js
meetingsession.closeStream(mediaStream)
```

**mediaStream** - response received from _generateStream_.
...
### Content Share Action [for web]

To share the screen during a meeting, the _contentShareAction_ function can be called to capture and share the screen.
This function allows participants to share their screen with others in the meeting. It can be used to present presentations, demos, or other visual content during the meeting.

```js
meetingsession.contentShareAction(audio, video)
```

Data type is as follows:

```js
contentShareAction((audio: boolean), (video: boolean))
```

**audio** - specifies whether the screenshare should capture audio. It is important to note that not all screen shares are able to capture audio, so this option ensures that audio is captured if it is available.

**video** - set video to true to capture the screenshare and false to stop capture.

To stop an ongoing screen share, false can be passed to both the audio and video parameters. It is not possible to start a screenshare by passing only audio as true and video as false, but it is possible to pass video as true and audio as false.

After the _contentShareAction_ function is called, the _updateParticipantList_ event is fired to update the local participant list. The streamReceived event is also received for remote participants when the stream is ready. The stream will not be received for the participant who is currently sharing the screen. This function allows participants to share their screen with others in the meeting.

### Content Share Action [only for Electron]

This function allows participants to share their screen with others in the meeting using Electron. It can be used to present presentations, demos, or other visual content during the meeting.

```js
meetingsession.contentShareAction(audio, video, options)
```

Data type of contentShareAction is as follows:
...
**video** - specifies whether the screenshare should capture video.

**options** - an object that contains a _chromeMediaSourceId_ field. The _chromeMediaSourceId_ field indicates the id of the screen that needs to be shared. In order to get the _chromeMediaSourceId_, some code needs to be added to the main process.
...
To capture the screen in Electron, more information can be found in the official Electron [documentation.](https://www.electronjs.org/docs/latest/api/desktop-capturer)
...
The listAvailableDevices function is used to retrieve a list of all available media devices. It returns an array of objects, each with details about a specific device.

```js
meetingsession.listAvailableDevices().then((res) => {
  // contains device list information
})
```

The returned data includes the device property, which holds the name of the device, the id property, which is a unique identifier for the device, and the type property, which specifies the type of media device.
...
**device** - contains the device name.

**id** - contains an unique id of the device.

**type** - defines which type of media device.

The deviceListModified event is fired when the currently used media device is changed. It is important to ensure that the currently used device in the SDK and on the client side are in sync. [deviceListModified](Electron#device-list-modified) event allows the client to track and update the media devices being used in the meeting.

### Switch Devices

The _switchDevices_ function is used to change the audio or video device being used in the meeting. To switch devices, the device id of the desired device can be passed to the _switchDevices_ function.
This allows the client to switch between different audio and video devices during a meeting.

```js
meetingsession.switchDevices(device_id, device, bindId)
```

The _switchDevices_ function is used to change the audio or video device being used in the meeting. It takes three parameters:

```js
switchDevices(
    device_id: string,
    device: "audioInput" | "audioOutput" | "videoInput",
    bindId?: string
)
```

**device_id** - unique identifier of the device that needs to be switched.

**device** - the type of device that needs to be switched (e.g. audio, video).

**bindId** - _(optional)_ and should only be passed when switching the audio output device, as a new stream needs to be bound to the audio element.

After the _switchDevices_ function is called, the deviceListModified event is fired, which returns the currently used devices. Note that it only returns the device that was changed. This event allows the client to update the list of currently used devices and ensure that the correct devices are being used in the meeting.

### Stream Bind

The _streamBind_ function is used to bind a stream to a specific video or audio element. When a stream is ready on the SDK, it will fire a _streamReceived_ event. When this event is received, the _streamBind_ function can be used to bind the stream to the desired element.
This allows the SDK to display streams from other participants in the meeting.

```js
meetingsession.streamBind(stream, participant_id, tile_id)
```

```js
streamBind(
  (stream: 'audio' | 'video' | 'screenshare'),
  (participant_id: string),
  (tile_id: string)
)
```

**stream** - specifies the type of stream that needs to be bound (e.g. video,audio,screen share).

**participant_id** - participant_id of participant whose stream needs to be bound. For audio streams, the participant_id will be the local participant's participant_id.

**tile_id** - indicates the DOM element id where the stream should be bound.

It is important to note that the DOM element should be inserted in the web page before calling the _streamBind_ function. This function allows the SDK to display streams from other participants in the meeting.

### Stream Unbind

If a user want to remove the streamObject from a DOM element , _streamUnbind_ can be used.

```js
meetingsession.streamUnbind(tile_id)
```

_streamUnbind_ type is as follows:

```js
streamUnbind((tile_id: string))
```

**tile_id** - indicates the DOM element id where stream needs to be removed.
...
Listeners should be added right after creating a ChatSession. When an event occurs concurrent callback function is called [Meeting events](Electron#meeting-events)
