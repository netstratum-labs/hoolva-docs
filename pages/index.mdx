...
### Observer
An observer is similar to an event listener that will trigger when a message is received. For convenience, we have grouped the Observer which throw similar messages. Eg, A participant observer will throw messages connected to the participants in the meeting.
...
|  API Rate                                                                |     10 requests per second (RPS) with a burst of 20 RPS. |  Yes                         |
 ...
 - [Hoolva SDK client library for JavaScript (NPM)](https://www.npmjs.com/package/hdmeet) – A JavaScript library with TypeScript type definitions that helps you build Hoolva SDK applications in WebRTC-enabled browsers.
